...
let _monacoReadyResolve = null;
let _monacoReadyReject = null;
const monacoReady = new Promise((resolve, reject) => {
    _monacoReadyResolve = resolve;
    _monacoReadyReject = reject;
});

// ═══════════════════════════════════════════════════════════════════════════
//  MONACO INITIALIZATION (Enhanced with proper error handling)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Initialize Monaco editor once it's loaded by the HTML script tag
 * Polls for the AMD loader and then triggers module load
 */
function initializeMonacoWait() {
    console.log("[PrepLoom] Starting Monaco load sequence...");

    // If Monaco is already available synchronously
    if (typeof window.monaco !== "undefined" && window.monaco.editor) {
        console.log("[PrepLoom] ✅ Monaco already loaded");
        _monacoReadyResolve(window.monaco);
        return;
    }

    let attempts = 0;
    const MAX_ATTEMPTS = 150; // 15 seconds total
    const CHECK_INTERVAL = 100;

    const pollInterval = setInterval(() => {
        attempts++;

        // Case 1: Monaco is already loaded
        if (typeof window.monaco !== "undefined" && window.monaco.editor) {
            clearInterval(pollInterval);
            console.log(`[PrepLoom] ✅ Monaco detected after ${attempts * CHECK_INTERVAL}ms`);
            _monacoReadyResolve(window.monaco);
            return;
        }

        // Case 2: AMD loader is available - trigger the load!
        if (typeof window.require === "function") {
            clearInterval(pollInterval);
            console.log("[PrepLoom] 🚀 AMD loader found, triggering module load...");
            
            window.require(['vs/editor/editor.main'], function() {
                console.log("[PrepLoom] ✅ Monaco modules loaded successfully via AMD");
                _monacoReadyResolve(window.monaco);
            }, function(err) {
                console.error("[PrepLoom] ❌ Monaco modules failed to load:", err);
                const error = new Error("[PrepLoom] Monaco module load failed: " + (err.message || "Unknown error"));
                _monacoReadyReject(error);
                showMonacoFallback(error);
            });
            return;
        }
        ...
        if (attempts % 10 === 0) {
            console.log(`[PrepLoom] Waiting for AMD loader... (${attempts * CHECK_INTERVAL}ms)`);
        }
        ...
        if (attempts >= MAX_ATTEMPTS) {
            ...
            const error = new Error(
                "[PrepLoom] Monaco editor failed to load after 15 seconds. " +
                "The AMD loader (require) never became available."
            );
            console.error(error);
            _monacoReadyReject(error);
            showMonacoFallback(error);
...
/**
 * Show fallback UI if Monaco fails
 */
function showMonacoFallback(error) {
    ...
    container.innerHTML = `
        ...
        ">
            ...
            <div style="font-size: 1.1rem; margin-bottom: 0.5rem; font-weight: 700;">Code Editor Unavailable</div>
            <div style="font-size: 0.9rem; color: rgba(255, 107, 107, 0.8); margin-bottom: 1rem;">
                ${error.message || "Monaco editor failed to load"}
            </div>
            <div style="font-size: 0.85rem; color: rgba(255, 107, 107, 0.7); line-height: 1.5;">
...
// Start waiting for Monaco as soon as this script loads
initializeMonacoWait();
...
function speak(text) {
    ...
    utter.rate = 0.95;
    utter.pitch = 1.0;
    if (tts.voices.length === 0) getVoices();
...
function resetInterviewUI() {
    lastAnswer = ""; answerDuration = 0; bodyLanguageData = null; liveTranscriptFinal = "";
    updateTranscript("");
    ...
    typeInput.value = "";
    stopLiveTranscript(); stopRecordingTimer(); setRecordingUI(false);
    hideCodeEditor();
    resetCodeEditor();
    if (isAnalyzingContinuous) stopContinuousAnalysis();
